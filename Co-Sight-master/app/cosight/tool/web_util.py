...
ProxySettings = None
ChatOpenAI = None
_browser_use_import_error: Optional[Exception] = None


def _ensure_browser_use_imports() -> None:
    global Agent, BrowserSession, BrowserProfile, ProxySettings, ChatOpenAI, _browser_use_import_error
    if Agent and BrowserSession and BrowserProfile and ProxySettings and ChatOpenAI:
        return
    if _browser_use_import_error is not None:
        raise RuntimeError(
            "browser_use dependency is unavailable in the current environment"
    ...
    try:
        ...
        BrowserProfile = getattr(browser_module, "BrowserProfile")
        ProxySettings = getattr(browser_module, "ProxySettings")
        ChatOpenAI = getattr(llm_module, "ChatOpenAI")
        logger.info("browser_use dependency loaded lazily")
    except Exception as ex:
        _browser_use_import_error = ex
        logger.error("browser_use dependency import failed: %s", ex, exc_info=True)
        raise RuntimeError(
            "browser_use dependency is unavailable in the current environment"
        ) from ex
