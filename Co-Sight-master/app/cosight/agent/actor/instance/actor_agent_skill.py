...
def legal_search_skill():
    return {
        'skill_name': 'legal_search',
        'skill_type': "function",
        'display_name_zh': '法律法规检索',
        'display_name_en': 'Legal Search',
        'description_zh': '检索中国法律法规、案例与本地知识库（得理法律 + 国家法律法规数据库 + Chroma），适用于合同、侵权、诉讼等法律研究任务',
        'description_en': 'Search Chinese laws, cases and local knowledge base',
        'semantic_apis': ["api_legal_search"],
        'function': SkillFunction(
            id='8f44f9ad-be5c-4e6c-a9d8-1426b23828b1',
            name='app.cosight.tool.legal_search_toolkit.legal_search',
            description_zh='混合法规检索，返回法规条文、案例摘要与本地知识库命中',
            description_en='Hybrid legal search across DeliLegal, NPC and local Chroma',
            parameters={
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description_zh": "法律检索关键词或问题",
                        "description_en": "Legal search query"
                    },
                    "limit": {
                        "type": "integer",
                        "description_zh": "返回条数，默认5",
                        "description_en": "Max results"
                    }
                },
                "required": ["query"]
            }
...
def register_mcp_tools():
    """读取 mcp_server_config.json 与管理端 custom_mcp_tools，避免误注册其它 JSON。"""
    from cosight_server.deep_research.services.admin_runtime_config import load_custom_mcp_tools

    config_path = os.path.join(mcp_server_config_dir, "mcp_server_config.json")
    raw = JsonUtil.read_data(config_path)
    tools: list = []
    if isinstance(raw, list):
        tools.extend(item for item in raw if isinstance(item, dict) and item.get("skill_name"))
    tools.extend(load_custom_mcp_tools())
    return tools
