...
from llm import get_llm_bundle_for_task
...
async def search(request: Request, params: Any = Body(None)):
    ...
    scenario = (params.get("scenario") or "").strip() if isinstance(params, dict) else ""
    if scenario and query_content:
        scenario_hints = {
            "contract_review": "【场景：合同审查】请重点关注条款风险、付款/违约/争议解决条款，并引用相关法规。",
            "legal_research": "【场景：法规研究】请优先使用法律法规检索工具，给出可追溯的法条依据。",
            "case_analysis": "【场景：类案分析】请归纳裁判要点与证据结构，区分事实与法律适用。",
            "document_draft": "【场景：文书起草】请生成结构化的法律文书草稿，标注待补充事实。",
            "evidence_check": "【场景：证据质检】请列出已有/缺失证据与证明力评估。",
        }
    ...
    async def generator_func():
        ...
        def run_manus():
            try:
                ...
                try:
                    from cosight_server.deep_research.services.legal_kb.task_backfill import maybe_backfill_after_task

                    backfill = maybe_backfill_after_task(query_content, plan_id=plan_id)
                    logger.info("Task knowledge backfill: %s", backfill)
                except Exception as backfill_exc:
                    logger.warning("Task knowledge backfill failed: %s", backfill_exc)
