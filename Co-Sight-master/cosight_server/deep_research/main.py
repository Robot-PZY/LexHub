...
try:
    ...
    startup_apply = apply_admin_runtime_config()
    logger.info(
        "管理端运行时配置已加载: models=%s apis=%s mcp=%s",
        startup_apply.get("modelsApplied"),
        startup_apply.get("apisApplied"),
        startup_apply.get("mcpToolCount"),
    )
except Exception as exc:
    logger.warning("管理端运行时配置加载跳过: %s", exc)
...
async def app_lifespan(_app: FastAPI):
    from cosight_server.deep_research.services.legal_kb.crawl_scheduler import start_scheduler, stop_scheduler
