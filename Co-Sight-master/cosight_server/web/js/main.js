...
function initInputHandler() {
  ...
  if (initialMessageInput && initialSendButton) {
    if (quickActionCards && quickActionCards.length > 0) {
      quickActionCards.forEach((card) => {
        card.addEventListener("click", () => {
          const prompt = card.getAttribute("data-prompt") || "";
          initialMessageInput.value = prompt;
          initialMessageInput.focus();
          initialMessageInput.style.height = "auto";
          initialMessageInput.style.height =
            Math.min(initialMessageInput.scrollHeight, 200) + "px";
        });
      });
    }
...
function initRightTabs() {
  ...
  const panels = document.querySelectorAll(".right-tab-panel");
  ...
  const statusMap = {
    summary: "查看案件摘要、争议焦点和任务目标...",
    evidence: "查看现有材料、关键证据和待补充清单...",
    authority: "查看法条依据、案例摘要和引用来源...",
    draft: "查看文书大纲、草稿建议和引用方向...",
  };
  ...
  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      if (status && target && statusMap[target]) {
        status.textContent = statusMap[target];
      }
    });
  });
}
