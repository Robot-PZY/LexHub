  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={isAdmin
          ? '从管理端观察任务闭环、智能体调度与 API 就绪情况，用于比赛演示与运维巡检。'
          : '基于 Co-Sight 任务闭环的运行数据概览，帮助评委快速理解系统调度与 API 就绪状态。'}
      ...
      {loading ? <LoadingState label="加载分析数据…" /> : (
        <>
          <section className="feature-stat-grid">
            <StatCard label="案件总数" value={`${data.summary.totalCases}`} description="含归档与进行中任务" />
            <StatCard label="高风险占比" value={`${data.summary.highRiskRatio}%`} description="触发强制审查比例" />
            <StatCard label="智能体调用" value={`${data.summary.agentCalls}`} description="累计调度次数" />
            <StatCard label="API 就绪率" value={`${data.summary.apiReadyRatio}%`} description="已配置 Key 的工具位" />
          </section>
