...
export function deriveDagNodes(steps: AgentStep[], toolCalls: ToolCallTrace[]): DagNode[] {
  if (steps.length === 0) {
    return [
      { id: 'intake', label: '任务理解', status: 'branch', hop: 1, agent: 'planner', summary: '识别场景与目标产出', tools: [] },
...
export function deriveAgentInvocations(steps: AgentStep[], toolCalls: ToolCallTrace[]) {
  ...
  toolCalls.forEach((tool) => touch(tool.agent, 'tool', `调用工具：${tool.toolName}`));
