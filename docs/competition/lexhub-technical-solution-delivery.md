...
本文是律枢 LexHub 的技术方案说明，对应国赛提交材料中的“技术方案文档”。文档从业务场景、系统架构、核心算法、可信安全、性能评估和运行演示几个方面展开，重点说明我们如何在中兴 Co-Sight 的多智能体底座上，做出一个面向法律业务的可运行系统。

LexHub 的定位是“基于中兴 Co-Sight 的法律业务智能服务平台”。项目没有把 Co-Sight 仅仅当作通用问答框架使用，而是围绕法律任务的实际流程，补充了场景入口、法律智能体、DAG 工作流、法规检索、文书导出和过程回放等能力。这样做的目标很明确：让系统不只会生成一段回答，而是能把材料、依据、过程和交付物放在同一条任务链路里处理。

<p align="center">表 0-1 文档覆盖范围</p>
...
<p align="center">表 0-2 项目实现范围概览</p>

| 实现方向 | 已落地内容 | 对应工程位置 |
|---|---|---|
| 法律任务工作台 | 场景选择、任务描述、材料上传、任务提交、执行页和结果页 | `LexHub/Co-Sight-master/cosight_frontend/src/pages/WorkspacePage.tsx`、`WorkspaceRunPage.tsx`、`WorkspaceResultPage.tsx` |
...
| 管理端能力 | 模型/API 配置、知识库维护、策略规则、用户管理、运行分析 | `LexHub/Co-Sight-master/cosight_frontend/src/pages/admin/` |
| 多智能体与 DAG | 六类法律智能体、条件触发节点、导出前审查和合规监测 | `LexHub/Co-Sight-master/config/agent-registry.json`、`LexHub/Co-Sight-master/config/legal-workflow.json` |
| 法律知识库 | 法规检索、本地向量库、NPC 法规接口、外部法律 API 集成位、任务知识回填 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/legal_kb/` |
| 可信与交付 | replay 解析、执行快照、审计链、文书导出、指标聚合 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/execution_snapshot.py`、`LexHub/Co-Sight-master/cosight_server/deep_research/services/audit_log.py`、`LexHub/Co-Sight-master/cosight_server/deep_research/services/document_export.py`、`LexHub/Co-Sight-master/cosight_server/deep_research/services/metrics_aggregator.py` |
| 演示材料 | 劳动争议、公司治理、数据合规三组材料与任务 prompt | `LexHub/test/` |

从目前的工程形态看，LexHub 已经不只是把 Co-Sight 换一个名字或界面。项目在前端页面、后端服务、法律知识库、工作流配置、审计回放和演示材料上都做了法律业务层的扩展。后续如果增加新场景，可以继续沿用这套结构：补测试材料、知识库种子、工作流条件和结果模板；如果增加新工具，则补 Actor 工具封装、后端服务和管理端配置入口。

## 1. 业务场景分析

### 1.1 目标场景

律枢 LexHub 面向的是法律业务中比较高频、也比较耗时的辅助工作，包括材料整理、法规研究、文书起草和过程复核。系统目前覆盖合同审查、劳动争议、公司治理、数据合规、法规研究、文书起草等任务，处理路径可以概括为：材料接入、任务拆解、法规检索、文书生成、交叉审查和审计归档。

我们选择法律行业作为场景，是因为这类任务很少是“一问一答”。一次普通的法律辅助任务，往往同时涉及事实材料、法律条文、类案线索、业务规则和最终文书。单次对话可以给出一个方向，但很难稳定覆盖完整过程。LexHub 的做法是把这些环节拆成可观察、可配置、可回放的任务节点，让系统承担整理、检索、生成和复核的辅助工作。

需要强调的是，LexHub 当前定位是“法律辅助”，不是替代律师或法务人员作出最终判断。对于合同审查、劳动争议、公司治理和数据合规等场景，系统输出事实摘要、证据缺口、法规依据、风险提示和文书初稿，最终结论仍然需要人工复核。这样的边界更符合真实法律业务对审慎性、可解释性和责任归属的要求。

<p align="center">表 1-1 目标场景与系统能力</p>
...
这些场景不是单独写在文档里的分类，而是和项目里的测试材料、页面入口、工作流配置对应起来的。其中，劳动争议、公司治理和数据合规已经准备了独立演示材料，适合现场用同一套流程跑出不同任务效果；合同审查和法规研究则更多体现系统的扩展空间，后续可以继续补充材料模板和知识库条目。

<p align="center">图 1-1 法律任务处理前后对比图</p>
...
<p align="center">图 1-2 LexHub 法律任务价值闭环图</p>
...
<p align="center">表 1-2 业务痛点与解决方式</p>
...
这些问题并不是缺少某一个工具，而是任务上下文被拆散了：材料放在文件夹里，法规散在检索网站里，结论留在聊天记录里，文书靠 Word 模板整理，复核记录又在人工备注中。LexHub 要解决的正是这种断裂感，把这些环节放回同一个工作区，让每个阶段的结果都能被保存、查看和复盘。

<p align="center">表 1-3 能力价值映射</p>
...
<p align="center">表 1-4 用户画像与核心需求</p>
...
不同用户对系统的关注点不完全一样。个人律师更看重单次任务的效率和文书初稿质量；律所团队更看重模板、知识库和办理流程能否统一；企业法务更关心批量合同或合规事项是否好管理；合规人员则更在意过程留痕和依据追溯。LexHub 用“任务工作台 + 管理端配置 + 回放归档”的结构把这些需求放在同一套产品里。

### 1.4 价值量化

<p align="center">表 1-5 价值量化指标</p>
...
这些数据用于说明演示环境下的对比效果，不作为生产环境承诺。实际效果会受到模型能力、知识库质量、案件复杂度、材料清晰度和人工复核标准影响。我们在文档中保留这些基准，是为了让评审能看到系统在哪些环节产生价值，而不是把它包装成固定不变的性能指标。

<p align="center">表 1-6 价值实现路径</p>

| 价值方向 | 实现路径 | 可验证证据 |
|---|---|---|
| 效率提升 | 材料接入、任务拆解、法规检索和初稿生成由工作流串联 | 工作台、DAG 执行页、测试材料 |
| 质量提升 | 事实、依据、风险、建议分阶段生成并在导出前复核 | 交叉审查智能体、结果页、导出文书 |
| 可信提升 | replay、执行快照和审计日志记录过程 | `/replay`、`LexHub/Co-Sight-master/cosight_server/deep_research/services/execution_snapshot.py`、`LexHub/Co-Sight-master/cosight_server/deep_research/services/audit_log.py` |
| 可扩展性 | 场景、智能体、工具和知识库均通过配置或服务扩展 | `LexHub/Co-Sight-master/config/legal-workflow.json`、`LexHub/Co-Sight-master/config/agent-registry.json`、`LexHub/Co-Sight-master/cosight_server/deep_research/services/legal_kb/` |
...
LexHub 的架构可以分为五层：用户交互层、后端服务层、智能体编排层、工具与知识层、可信与数据层。前端负责把任务提交、材料上传、执行过程和结果页呈现出来；后端负责接入 Co-Sight 执行过程，并处理材料、知识库、文书导出和审计接口；智能体层根据任务状态决定下一步执行；工具层提供搜索、法规检索、文档处理、代码执行和导出能力；可信层保存 replay、执行快照和审计日志。

我们没有重写 Co-Sight 的底层执行框架，而是把它作为通用智能体底座使用。LexHub 在此基础上增加法律场景的工作流配置、工具封装、知识库服务和展示页面。这样做的好处是边界清楚：底层执行能力由 Co-Sight 承担，法律行业逻辑则通过配置和服务模块持续扩展。

系统运行时，用户先在前端提交任务描述、场景类型和材料；后端把材料整理到任务工作区，并触发 Co-Sight 执行；执行过程中产生的阶段事件、工具调用、文件产物和模型输出会进入 replay；任务结束后，结果页读取执行快照并组织结构化展示，同时提供文书导出和回放复核入口。

<p align="center">图 2-1 系统总体架构图</p>
...
<p align="center">表 2-1 核心工程模块</p>

| 模块 | 路径 | 作用 |
|---|---|---|
| 前端用户端 | `LexHub/Co-Sight-master/cosight_frontend/src/pages/` | 工作台、任务执行、结果页、材料库、回放页 |
| 前端管理端 | `LexHub/Co-Sight-master/cosight_frontend/src/pages/admin/` | 模型/API、知识库、策略、用户管理 |
| 后端 API | `LexHub/Co-Sight-master/cosight_server/deep_research/routers/` | 上传、知识库、文书、审计等接口 |
| 法律知识库 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/legal_kb/` | 法规、模板、类案检索 |
| 法律工具 | `LexHub/Co-Sight-master/app/cosight/tool/legal_search_toolkit.py` | Co-Sight Actor 可调用的法律检索工具 |
| 工作流配置 | `LexHub/Co-Sight-master/config/legal-workflow.json` | 法律任务 DAG 编排 |
| 智能体注册 | `LexHub/Co-Sight-master/config/agent-registry.json` | 智能体角色、工具、触发条件 |

其中，`LexHub/Co-Sight-master/config/legal-workflow.json` 与 `LexHub/Co-Sight-master/config/agent-registry.json` 是系统从通用 Co-Sight 迁移到法律场景的核心配置。前者描述任务如何流转，后者描述有哪些智能体、它们具备什么能力、能调用哪些工具。两者结合后，系统才能在运行时解释“为什么触发某个节点、为什么调用某类工具、为什么进入导出前复核”。

<p align="center">表 2-2 前端功能页面</p>
...
| 可信评估 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/credibility_analyzer.py`、`LexHub/Co-Sight-master/cosight_server/deep_research/services/audit_log.py` | 支撑可信分级、审计链和过程复核 |
| 执行归档 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/execution_snapshot.py`、`LexHub/Co-Sight-master/cosight_server/deep_research/services/metrics_aggregator.py` | 从 replay 中提取执行快照、阶段统计和运行指标 |
| 文书导出 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/document_export.py` | 支撑 Markdown、DOCX、PDF 等导出链路 |

<p align="center">表 2-4 后端接口能力矩阵</p>
...
<p align="center">表 2-5 前端路由与权限映射</p>
...
前端和后端的分工比较直接：用户端把法律任务变成可操作流程，管理端维护模型、API 和知识库能力，后端服务把 Co-Sight 执行、法律知识检索和文书导出接在一起。这样的结构方便现场演示，也方便后续增加更多法律场景。

### 2.2 智能体角色分工

LexHub 将法律任务拆成 6 类智能体协同处理。它们不是按固定顺序机械执行，而是根据任务描述、材料完整度、法规引用覆盖率、风险等级和用户目标触发。

角色设计遵循三个层次：任务理解智能体负责调度，证据质检、法规研究、文书生成智能体负责执行，交叉审查和合规监测智能体负责复核。这样可以避免所有能力堆在一个提示词或一个模型调用中，也便于在前端展示每个阶段的输入、输出和责任边界。

<p align="center">表 2-6 智能体角色分工</p>
...
<p align="center">图 2-2 多智能体协作流程图</p>
...
<p align="center">代码 2-1 智能体注册表节选</p>

<p align="center">来源：`LexHub/Co-Sight-master/config/agent-registry.json`</p>
...
系统通过 `LexHub/Co-Sight-master/config/legal-workflow.json` 定义法律任务 DAG。典型链路为：
...
<p align="center">图 2-3 法律任务 DAG 编排图</p>
...
<p align="center">表 2-7 DAG 分支规则</p>
...
<p align="center">代码 2-2 法律工作流配置节选</p>

<p align="center">来源：`LexHub/Co-Sight-master/config/legal-workflow.json`</p>
...
工具集成分为三类：一类是 Co-Sight 原生工具，如搜索、文件处理和代码执行；一类是法律业务工具，如法规检索、知识库和文书模板；还有一类是可信交付工具，如 replay、审计日志和导出快照。工具调用结果会进入任务执行记录，后续结果页、导出文书和回放页面都可以使用这些记录。

<p align="center">表 2-8 工具/API 集成方案</p>

| 能力 | 实现 | 作用 |
|---|---|---|
| 联网搜索 | Tavily / Google / Search API 集成位 | 补充公开资料和主体信息 |
| 法规检索 | `LexHub/Co-Sight-master/app/cosight/tool/legal_search_toolkit.py`、本地 Chroma、NPC 法规、得理法律 API 集成位 | 检索法规、案例、模板 |
...
<p align="center">图 2-4 法规检索与知识融合流程图</p>
...
<p align="center">代码 2-3 法律检索工具节选</p>

<p align="center">来源：`LexHub/Co-Sight-master/app/cosight/tool/legal_search_toolkit.py`</p>
...
<p align="center">图 2-5 法律知识来源与入库用途图</p>
...
<p align="center">表 2-9 法律知识来源治理</p>
...
<p align="center">代码 3-1 任务状态指标示例</p>
...
<p align="center">表 3-2 智能体协作机制</p>
...
<p align="center">代码 3-2 Replay 执行快照解析节选</p>

<p align="center">来源：`LexHub/Co-Sight-master/cosight_server/deep_research/services/execution_snapshot.py`</p>
...
在实现上，我们没有追求复杂算法堆叠，而是优先保证工程可控。第一，减少不必要的节点，让执行链路保持清楚；第二，在关键节点保留依据和中间结果，让输出可以追溯；第三，在导出前增加复核门槛，避免生成内容未经检查就进入交付。法律任务的风险往往来自事实不清、依据缺失和过程不可查，而不只是模型“会不会写”的问题。

<p align="center">表 3-4 核心算法与工程实现对应关系</p>

| 算法/机制 | 工程实现 | 输出结果 |
|---|---|---|
| 状态驱动任务拆解 | `LexHub/Co-Sight-master/config/legal-workflow.json` 中的 condition 规则 | 动态选择证据质检、法规研究、文书生成和审查节点 |
| 多智能体职责分配 | `LexHub/Co-Sight-master/config/agent-registry.json` 中的 role、capabilities、registeredTools | 形成调度者、工作者、审查者三层协作结构 |
| 法规 RAG 检索 | `LexHub/Co-Sight-master/app/cosight/tool/legal_search_toolkit.py`、`LexHub/Co-Sight-master/cosight_server/deep_research/services/legal_kb/legal_search.py` | 法规、案例、模板和来源摘要 |
| Replay 快照解析 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/execution_snapshot.py` | 阶段、工具、统计、来源和导出段落 |
| 审计链生成 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/audit_log.py` | 审计条目、链式 hash、归档摘要 |
| 指标聚合 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/metrics_aggregator.py` | 任务耗时、工具调用、DAG 跳数、引用追溯率 |

<p align="center">代码 3-3 性能指标聚合节选</p>

<p align="center">来源：`LexHub/Co-Sight-master/cosight_server/deep_research/services/metrics_aggregator.py`</p>
...
可信安全设计主要回答三个问题：材料放在哪里，权限如何分开，过程能不能查。当前项目面向国赛演示环境，重点实现本地化存储、配置隔离和过程审计；如果进入生产部署，可以在此基础上继续接入服务端鉴权、租户隔离、脱敏处理和审计日志持久化。

### 4.1 数据保护措施

<p align="center">表 4-1 数据保护措施</p>
...
<p align="center">表 4-2 数据生命周期设计</p>
...
<p align="center">表 4-4 管理端配置边界</p>
...
<p align="center">图 4-1 可信审计链路图</p>
...
<p align="center">表 4-6 可信安全代码证据</p>

| 可信环节 | 代码或配置位置 | 说明 |
|---|---|---|
| 审计链生成 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/audit_log.py` | 从 replay 事件构造审计条目，并通过前序 hash 与当前 payload 生成链式摘要 |
| 执行快照解析 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/execution_snapshot.py` | 将 replay 解析为阶段、工具、结果、来源和导出附录 |
| 审计接口 | `LexHub/Co-Sight-master/cosight_server/deep_research/routers/common.py` | 提供 `/demo/audit-log`、`/demo/execution-snapshot`、`/demo/export-document` 等接口 |
| 权限边界 | `LexHub/Co-Sight-master/cosight_frontend/src/App.tsx` | 用户端路由与管理端路由通过 `ProtectedRoute role` 分离 |
| 工具注册 | `LexHub/Co-Sight-master/config/agent-registry.json` | 将 `audit_log`、`export` 等能力纳入工具目录，供合规监测智能体调用 |
| 导出门禁 | `LexHub/Co-Sight-master/config/legal-workflow.json` | `review` 与 `compliance` 节点在高风险或导出前触发 |

<p align="center">代码 4-1 审计链 hash 生成节选</p>

<p align="center">来源：`LexHub/Co-Sight-master/cosight_server/deep_research/services/audit_log.py`</p>
...
审计机制让评审人员看到系统从输入材料到最终结论的过程，而不只是看到一份文书。对于法律任务，这一点很关键：如果结论有争议，可以回到 replay 查看引用、工具调用和阶段输出，判断问题出在材料理解、法规检索、内容生成还是复核阶段。
...
| 测试材料 | `LexHub/test/case-01-labor-dispute/`、`LexHub/test/case-02-corporate-governance/`、`LexHub/test/case-03-data-compliance/` |
| 评估方式 | 同一任务分别对比传统人工处理流程与 LexHub 演示流程 |

<p align="center">表 5-2 测试任务设计</p>
...
<p align="center">图 5-2 端到端任务指标对比图</p>
...
<p align="center">表 5-5 性能评估数据来源</p>

| 数据来源 | 生成方式 | 用途 |
|---|---|---|
| 演示基准数据 | 基于测试材料与预期演示流程整理 | 在比赛文档中说明相对提升方向 |
| replay 事件 | Co-Sight 执行过程自动记录 | 统计阶段、工具调用、消息数和回放覆盖 |
| execution snapshot | 由 `LexHub/Co-Sight-master/cosight_server/deep_research/services/execution_snapshot.py` 从 replay 解析 | 支撑结果页、文书导出和过程溯源 |
| metrics aggregator | 由 `LexHub/Co-Sight-master/cosight_server/deep_research/services/metrics_aggregator.py` 聚合 replay | 计算平均耗时、工具调用、DAG 跳数和引用追溯率 |
| 人工复核记录 | 法律专业人员对结果进行检查 | 校验事实、依据和风险提示是否可用 |

### 5.3 对比分析

从测试结果看，LexHub 的提升主要来自两点：一是流程自动化，二是过程可追溯。传统方式下，材料阅读、法规检索、结论整理和文书初稿通常由人工连续完成，过程记录也比较分散；LexHub 将这些步骤拆成可编排节点，再由多智能体协同推进，处理过程更容易观察和复核。
...
功能演示建议围绕一次完整法律任务展开，从任务提交讲到结果交付。劳动争议材料适合作为主演示案例，因为它同时包含解除通知、考勤记录、工资明细和仲裁草稿，能够展示材料分析、法规研究、文书生成和审计回放等能力。

<p align="center">图 6-1 功能演示闭环图</p>
...
<p align="center">表 6-2 演示测试材料</p>

| 目录 | 场景 | 展示重点 |
|---|---|---|
| `LexHub/test/case-01-labor-dispute/` | 劳动争议：解除、工资、考勤、仲裁 | 适合展示完整办案链路 |
| `LexHub/test/case-02-corporate-governance/` | 公司治理：董事辞任、股权结构、股东会决议 | 适合展示主体关系和治理合规分析 |
| `LexHub/test/case-03-data-compliance/` | 数据合规：个人信息保护、SDK 披露、监管整改 | 适合展示合规类任务和法规依据 |

<p align="center">表 6-3 测试材料文件构成</p>
...
<p align="center">表 6-5 运行证据类型</p>

| 证据类型 | 对应内容 | 证明重点 |
|---|---|---|
| 页面运行证据 | 工作台、执行页、结果页、回放页 | 系统具备完整操作链路 |
| 配置证据 | `LexHub/Co-Sight-master/config/agent-registry.json`、`LexHub/Co-Sight-master/config/legal-workflow.json` | 智能体角色与 DAG 不是静态描述 |
...
<p align="center">表 A-1 可运行代码与模型交付物</p>

| 赛题要求 | 项目交付物 |
|---|---|
| Co-Sight 工作流配置文件 | `LexHub/Co-Sight-master/config/legal-workflow.json` |
| 自定义工具封装源代码 | `LexHub/Co-Sight-master/app/cosight/tool/legal_search_toolkit.py` |
| 知识库与规则库文件 | `LexHub/Co-Sight-master/config/knowledge/`、`LexHub/Co-Sight-master/work_space/knowledge_store/` |
| 部署脚本与配置文件 | `LexHub/start-lexhub.bat`、`LexHub/Co-Sight-master/.env_template`、`LexHub/Co-Sight-master/config/runtime/` |
| 前端工程 | `LexHub/Co-Sight-master/cosight_frontend/` |
| 后端工程 | `LexHub/Co-Sight-master/cosight_server/` |

<p align="center">表 A-2 核心配置与代码证据</p>

| 能力 | 文件 | 关键内容 |
|---|---|---|
| 智能体角色注册 | `LexHub/Co-Sight-master/config/agent-registry.json` | 定义 planner、evidence、research、drafting、review、compliance 六类智能体及其工具能力 |
| 法律 DAG 编排 | `LexHub/Co-Sight-master/config/legal-workflow.json` | 定义任务理解、证据质检、法规研究、文书生成、交叉审查、合规监测等节点和条件边 |
| 法律检索工具 | `LexHub/Co-Sight-master/app/cosight/tool/legal_search_toolkit.py` | 将法律 RAG 检索封装为 Co-Sight Actor 可调用工具 |
| 知识库服务 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/legal_kb/` | 承载法规、模板、类案和本地向量检索能力 |
| 执行快照 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/execution_snapshot.py` | 将 replay 事件解析为可展示、可导出、可审计的结构化快照 |
| 审计日志 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/audit_log.py` | 基于 replay 事件生成链式审计记录 |
| 文书导出 | `LexHub/Co-Sight-master/cosight_server/deep_research/services/document_export.py` | 生成 DOCX/PDF 等交付文档 |

<p align="center">代码 A-1 工作流工具注册节选</p>
...
<p align="center">代码 A-2 智能体工具目录节选</p>
...
<p align="center">表 A-3 图表与数据复现文件</p>
...
<p align="center">表 B-1 法律知识库参考来源</p>
...
<p align="center">表 B-2 赛题与技术参考资料</p>
